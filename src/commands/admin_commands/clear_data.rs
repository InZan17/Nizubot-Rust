...
pub async fn clear_data(ctx: Context<'_>) -> Result<(), Error> {
    if let Some(guild_id) = ctx.guild_id() {
        ctx.data().storage_manager.delete_data(vec!["guilds", guild_id.to_string().as_str()]).await;
        ctx.reply("Successfully removed guild data.").await?;
    } else {
        ctx.data().storage_manager.delete_data(vec!["users", ctx.author().id.to_string().as_str()]).await;
