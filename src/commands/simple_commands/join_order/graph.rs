...
use poise::{
    serenity_prelude::{CreateAttachment, CreateEmbed, CreateEmbedFooter},
...
#[derive(poise::ChoiceParameter, PartialEq, Clone, Copy)]
pub enum GraphData {
    #[name = "Total members"]
    TotalMembers,
    #[name = "New members"]
...
pub async fn graph(
    ...
    let end_date_string = {
        let time = get_time_string(end_date, time_format);
        format!(
            "{}-{}-{} {}",
            end_date.year(),
            end_date.month(),
    ...
    let handle = {
        let start_date_string = start_date_string.clone();
        let end_date_string = end_date_string.clone();
        ...
        spawn_blocking(move || {
                    ...
                    GraphType::BarGraph => {
                        chart.draw_series(
                            Histogram::vertical(&chart)
                                ...
                                .data(amount_during.iter().enumerate().map(|(x, y)| {
                                    (x.saturating_sub(1), if x == 0 { 0 } else { *y })
                                })),
                        )?;
                    }
                }
            ...
            let image =
                RgbImage::from_vec(width, height, buffer).ok_or("Failed to create RgbImage")?;
            ...
            Ok::<_, Error>(png_bytes)
        })
    };

    let png_bytes = handle.await??;
