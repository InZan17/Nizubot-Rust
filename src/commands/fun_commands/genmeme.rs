...
pub async fn caption(
    ...
    #[description = "What the bottom text should be. (type \"\\n\" to make a new line.)"] bottom_text: Option<String>,
    #[description = "Size of the font. (WHAT: width/7, Boxes: width/10, Overlay: height/10)"] font_size: Option<String>,
    #[description = "How big the space between new lines should be. (Default: fontsize/4)"] break_height: Option<String>,
    #[description = "Amount of empty space around the text. (WHAT: width/9, Boxes: width/20, Overlay: height/30)"] padding: Option<String>,
) -> Result<(), Error> {
    ...
    if upper_text.is_none() && bottom_text.is_none() {
        ctx.send(|m| {
            m.content("Please provide some text.")
        }).await?;
        return Ok(())
    }
    ...
    let font_size_expression = font_size.unwrap_or_else(|| {
        match caption_type {
    ...
    if font_size < 0. {
        ctx.send(|m| {
            m.content("Please make sure \"font_size\" isn't a negative number.")
        }).await?;
        return Ok(())
    }
    ...
    let break_height_expression = break_height.unwrap_or_else(|| {
        "fontsize/4".to_owned()
    });
    ...
    if break_height < 0. {
        ctx.send(|m| {
            m.content("Please make sure \"break_height\" isn't a negative number.")
        }).await?;
        return Ok(())
    }
    ...
    let padding_expression = padding.unwrap_or_else(|| {
        match caption_type {
    ...
    if padding < 0. {
        ctx.send(|m| {
            m.content("Please make sure \"padding\" isn't a negative number.")
        }).await?;
        return Ok(())
    }
    ...
    match caption_type {
        CaptionType::Boxes => {
            if bottom_texts.len() > 0 {
                let bottom_height = padding*2. + font_size*(bottom_texts.len() as f64) + break_height*(bottom_texts.len() as f64 -1.);
                ffmpeg_filter = format!("{ffmpeg_filter}pad=width=iw:height=ih+{bottom_height}:x=0:y=0:color=0xFFFFFF,")
            }


            if upper_texts.len() > 0 {
                let upper_height = padding*2. + font_size*(upper_texts.len() as f64) + break_height*(upper_texts.len() as f64 -1.);
        ...
        CaptionType::What => {
            ...
            let bottom_height = padding + font_size * bottom_texts.len() as f64 + break_height * (bottom_texts.len() as f64 - 1.).max(0.);
            let upper_height = padding + font_size * upper_texts.len() as f64 + break_height * (upper_texts.len() as f64 - 1.).max(0.);
            ...
            ffmpeg_filter = format!("{ffmpeg_filter}pad=width=iw:height=ih+{}:y={upper_height}:color=0x000000,", upper_height+bottom_height);

            ffmpeg_filter = format!("{ffmpeg_filter}pad=width=ih*({width}/{height}):x=(iw-out_w)/2:color=0x000000,");
        },
        CaptionType::Overlay => {}
    }
    ...
    for (index, text) in bottom_texts.into_iter().enumerate() {
        ...
        let alignment_offset = format!("-max_glyph_a-{}",font_size-font_ascent-font_descent/2.);
        let line_offset = padding + (font_size + break_height) * (bottom_texts_len - 1 - index ) as f64;
    ...
    ctx.send(|m| {
        m.attachment(AttachmentType::File { file: &generated_image_file, filename: format!("{}_{}.{}", image.filename, caption_type.to_string(), extension) })
    }).await?;
